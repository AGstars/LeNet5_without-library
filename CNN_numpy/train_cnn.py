...
def cnn_forward(input):
	...
	pool_out = pool_out
	flatten_out = pool_out.reshape(pool_out.shape[0],pool_out.shape[1]* pool_out.shape[2] * pool_out.shape[3])
...
if __name__ == '__main__':
	...
	mnist_train = pd.read_csv("C://OpenSource//numpy-for-CNN-master//mnist_dataset//mnist_train_100.csv",header=None)
	train_x = np.array(mnist_train.values[ : ,1:785])
	y_train = np.array(mnist_train.values[ : ,0])
	train_y = translate(y_train)

	# 读取测试数据
	mnist_test = pd.read_csv('C://OpenSource//numpy-for-CNN-master//mnist_dataset//mnist_test_10.csv',header=None)
