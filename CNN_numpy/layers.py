            ...
            for c in np.arange(f_out_channel):
                for h in np.arange(out_height):
                    for w in np.arange(out_width):
                        if bias == True:
                            out[b,c,h // self.stride,w // self.stride] = np.sum(img[b,:,h:h + f_height,w:w + f_width] *weight[:,c]) + self.bias[c]
                        elif bias == False:
            ...
            for c in np.arange(channel):
                for h in np.arange(out_h):
                    for w in np.arange(out_w):
                        pool_out[b,c,h,w] = np.max(img[b,c, self.stride * h : self.stride * h + self.pool_h,self.stride * w : self.stride * w + self.pool_w])
        return pool_out
    ...
    def pool_backward(self,dout):
        ...
        for b in np.arange(batch_size):
            for c in np.arange(channel):
                for h in np.arange(out_h):
                    for w in np.arange(out_w):
                        max_index = np.argmax(self.x[b,c, self.stride * h: self.stride * h + self.pool_h, self.stride * w: self.stride * w + self.pool_w])
